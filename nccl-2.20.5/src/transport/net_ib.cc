      ...
      for (int d=0; d<nIbDevs && ncclNIbDevs<MAX_IB_DEVS; d++) {
        ...
        for (int port_num = 1; port_num <= devAttr.phys_port_cnt; port_num++) {
          ...
          if (portAttr.state != IBV_PORT_ACTIVE){
            WARN("NET/IB:device :%s port :%d is unactive",devices[d]->name,port_num);
            continue;
          } 
          if (portAttr.link_layer != IBV_LINK_LAYER_INFINIBAND
              && portAttr.link_layer != IBV_LINK_LAYER_ETHERNET) {
            WARN("NET/IB:device :%s port :%d link_lay is error",devices[d]->name,port_num);    
            continue;
          }
        ...
        NCCLCHECK(wrap_ibv_query_gid(context, port_num, 0, &gid));
