...
ncclResult_t ncclTopoGetXmlFromSys(struct ncclXmlNode* pciNode, struct ncclXml* xml) {
  ...
  if (path)
  {
    ...
    float currentSpeed = FLT_MAX;

    NCCLCHECK(ncclTopoGetStrFromSys(path,"current_link_speed",strValue));
    sscanf(strValue, "%f GT/s", &currentSpeed);    
    NCCLCHECK(ncclTopoGetStrFromSys(path,"max_link_speed",strValue));
    sscanf(strValue, "%f GT/s", &maxSpeed);    
    if (currentSpeed<maxSpeed)
    {
      WARN("PCI: PCI_node busId : %s,current_link_speed!=max_link_speed,current_link_speed=%f,max_link_speed=%f", busId,currentSpeed,maxSpeed);
    }
  }
  //width
  if (path)
  {
    ...
    int maxLinkWidth = strtol(strValue, NULL, 0);
    NCCLCHECK(ncclTopoGetStrFromSys(path,"current_link_width",strValue));
    int currentLinkWidth = strtol(strValue, NULL, 0);
    if (currentLinkWidth<maxLinkWidth)
    {
      WARN("PCI:PCI_node busId : %s,currentLinkWidth!=maxLinkWidth,currentLinkWidth=%d,maxLinkWidth=%d", busId,currentLinkWidth,maxLinkWidth);
    }
  }
  if (path) {
...
ncclResult_t ncclTopoGetXmlFromGpu(struct ncclXmlNode* pciNode, nvmlDevice_t nvmlDev, struct ncclXml* xml, struct ncclXmlNode** gpuNodeRet, int rank_num) {
  ...
  if (nvlNode == NULL) {
    ...
    for (int l=0; l<maxNvLinks; ++l) {
      ...
      if (isActive != NVML_FEATURE_ENABLED){
        WARN("NVLINK: rank: %d Nvlink: %d unacive",rank_num,l);
        continue;
      } 
...
ncclResult_t ncclTopoFillGpu(struct ncclXml* xml, const char* busId, struct ncclXmlNode** gpuNode,int rank_num) {
  ...
  NCCLCHECK(ncclTopoGetXmlFromGpu(node, nvmlDev, xml, gpuNode, rank_num));
  return ncclSuccess;
}
